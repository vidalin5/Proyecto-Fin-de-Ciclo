...
/**
 * \file    test/phpunit/RecursosHumanosFunctionalTest.php
...
/**
 * Class RecursosHumanosFunctionalTest
 *
 ...
 *
 * @package Testrecursoshumanos
 */
class RecursosHumanosFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modRecursosHumanos')->click();
		} else {
			// Disable the module
			$this->byHref('modRecursosHumanos')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/recursoshumanos/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('recursoshumanos/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/recursoshumanos/admin/about.php');
		$this->authenticate();
		return $this->assertContains('recursoshumanos/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('recursoshumanoswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modRecursosHumanos_RecursosHumanosTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modRecursosHumanos_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
