	...
	if (GETPOST('nom', 'nohtml')) {
		...
		for ($i = 0; $i < $nbcolonnes; $i++) {
			if (GETPOSTISSET("choix$i") && GETPOST("choix$i") == '1') {
				...
				$nouveauchoix .= "2";
			} elseif (GETPOSTISSET("choix$i") && GETPOST("choix$i") == '3') {
				$nouveauchoix .= "3";
			} elseif (GETPOSTISSET("choix$i") && GETPOST("choix$i") == '4') {
				$nouveauchoix .= "4";
			} elseif (GETPOSTISSET("choix$i") && GETPOST("choix$i") == '5') {
				$nouveauchoix .= "5";
			} else {
				$nouveauchoix .= "0";
			}
...
while ($compteur < $num) {
	...
	if (!$testligneamodifier) {
		for ($i = 0; $i < $nbcolonnes; $i++) {
			...
			if (empty($listofanswers[$i]['format']) || !in_array($listofanswers[$i]['format'], array('yesno', 'foragainst', 'notas'))) {
				if (((string) $car) == "1") {
			...
			//PROBANDO
			if (!empty($listofanswers[$i]['format']) && $listofanswers[$i]['format'] == 'notas') {
				if (((string) $car) == "0") {
					print '<td class="non">1</td>'."\n";
				} elseif (((string) $car) == "1") {
					print '<td class="non">2</td>'."\n";
				} elseif (((string) $car) == "3") {
					print '<td class="ok">3</td>'."\n";
				} elseif (((string) $car) == "4") {
					print '<td class="ok">4</td>'."\n";
				} elseif (((string) $car) == "5") {
					print '<td class="ok">5</td>'."\n";
				} else {
					print '<td class="vide">&nbsp;</td>'."\n";
				}
				// Total
				...
				if (!isset($sumcuatro[$i])) {
					$sumcuatro[$i] = 0;
				}
				if (!isset($sumcinco[$i])) {
					$sumcinco[$i] = 0;
				}
				if (!isset($sumtotalvot)) {
					$sumtotalvot = 0;
				}
				if (((string) $car) == "1") {
					$sumdos[$i]++;
		...
		if ($compteur == $ligneamodifier) {
			for ($i = 0; $i < $nbcolonnes; $i++) {
				...
				if (!empty($listofanswers[$i]['format']) && $listofanswers[$i]['format'] == 'notas') {
					$arraychoice = array('2'=>'&nbsp;', '0'=>'1', '1'=>'2', '3'=>'3', '4'=>'4', '5'=>'5');
...
if ($object->allow_spy) {
	...
	$sumtotalvot = $sumtotalvot / $nbcolonnes;

	for ($i = 0; $i < $nbcolonnes; $i++) {
		$showsumfor = isset($sumfor[$i]) ? $sumfor[$i] : '';
		$showsumagainst = isset($sumagainst[$i]) ? $sumagainst[$i] : '';
		$showsumuno = isset($sumuno[$i]) ? $sumuno[$i] : '';
		$showsumdos = isset($sumdos[$i]) ? $sumdos[$i] : '';
		$showsumtres = isset($sumtres[$i]) ? $sumtres[$i] : '';
		$showsumcuatro = isset($sumcuatro[$i]) ? $sumcuatro[$i] : '';
		$showsumcinco = isset($sumcinco[$i]) ? $sumcinco[$i] : '';
		if (empty($showsumfor)) {
			$showsumfor = 0;
		}
		...
		if (empty($showsumtres)) {
			$showsumtres = 0;
		}
		if (empty($showsumcuatro)) {
			$showsumcuatro = 0;
		}
		if (empty($showsumcinco)) {
			$showsumcinco = 0;
		}
		...
		if (!empty($listofanswers[$i]['format']) && $listofanswers[$i]['format'] == 'notas') {
			if (empty($sumtotalvot)) {
				$sumtotalvot = 0;
			} else {
				$sumtotalvot = $sumtotalvot;
			}
	
			if (empty($sumcolumna[$i])) {
				$sumcolumna[$i] = 0;
			}
	
			if (!empty($sumtotalvot) && !empty($sumcolumna[$i])) {
				$media = number_format($sumcolumna[$i]/$sumtotalvot, 2);
			} else {
				$media = 0;
			}
			...
			print '<b>Nota 1</b>: '.$showsumuno.'<br><b>Nota 2</b> :'.$showsumdos.'<br><b>Nota 3</b>: '.$showsumtres.'<br><b>Nota 4</b> :'.$showsumcuatro.'<br><b>Nota 5</b> :'.$showsumcinco.'<br><br>Total votos: '.$sumtotalvot.'<br>Suma col: '.$sumcolumna[$i].'<br><br><b><div style="background-color:'.$color.'">MEDIA: '.$media.'</div></b>';
		}
		print '</td>'."\n";
	}
	print '</tr>';
	...
	if ($nbofcheckbox >= 2) {
		...
		for ($i = 0; $i < $nbcolonnes; $i++) {
			...
			if (empty($listofanswers[$i]['format']) || !in_array($listofanswers[$i]['format'], array('yesno', 'foragainst', 'notas')) && isset($sumfor[$i]) && isset($meilleurecolonne) && $sumfor[$i] == $meilleurecolonne) {
