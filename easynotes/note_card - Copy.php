...
// Part to create
if ($action == 'create') {
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Note")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Note"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->easynotes->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $user->rights->easynotes->note->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->easynotes->note->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('easynotes:Note', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('note'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/easynotes/note_agenda.php', 1).'?id='.$object->id.'">';
	...
	$diroutput = $conf->easynotes->dir_output;
	$trackid = 'note'.$object->id;
