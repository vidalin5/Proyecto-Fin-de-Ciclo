...
/**
 * \file    test/phpunit/EasyNotesFunctionalTest.php
 * \ingroup easynotes
 ...
 *
 * @package Testeasynotes
 */
class EasyNotesFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modEasyNotes')->click();
		} else {
			// Disable the module
			$this->byHref('modEasyNotes')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/easynotes/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('easynotes/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/easynotes/admin/about.php');
		$this->authenticate();
		return $this->assertContains('easynotes/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('easynoteswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modEasyNotes_EasyNotesTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modEasyNotes_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
