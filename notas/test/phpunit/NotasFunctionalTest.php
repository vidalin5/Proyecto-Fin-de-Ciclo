...
/**
 * \file    test/phpunit/NotasFunctionalTest.php
 * \ingroup notas
 ...
 * @package Testnotas
 */
class NotasFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	{
		$this->url('/custom/notas/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('notas/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/notas/admin/about.php');
		$this->authenticate();
		return $this->assertContains('notas/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('notaswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modNotas_NotasTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modNotas_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
